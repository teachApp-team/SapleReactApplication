import { Base, IList$1, IEnumerator$1, IEnumerator, Type } from "igniteui-react-core";
/**
 * @hidden
 */
export declare class SafeSortedReadOnlyDoubleCollection extends Base implements IList$1<number> {
    static $t: Type;
    private a;
    private b;
    private c;
    d(a: number): number;
    constructor(a: number, b: IList$1<number>, c: IList$1<number>);
    constructor(a: number, b: IList$1<number>, c: IList$1<number>, d: number);
    constructor(a: number, ..._rest: any[]);
    indexOf(a: number): number;
    insert(a: number, b: number): void;
    removeAt(a: number): void;
    item(a: number, b?: number): number;
    add(a: number): void;
    clear(): void;
    contains(a: number): boolean;
    copyTo(a: number[], b: number): void;
    readonly count: number;
    readonly isReadOnly: boolean;
    remove(a: number): boolean;
    private _getEnumerator;
    getEnumerator(): IEnumerator$1<number>;
    getEnumeratorObject(): IEnumerator;
}
