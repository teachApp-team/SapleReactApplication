...
export function stringFormat(format, ...rest) {
    return stringFormat1(format, ...rest);
}
;
export function stringFormat1(format, ...args) {
    return stringFormat2(CultureInfo.currentCulture, format, ...args);
}
;
export function stringFormat2(provider, format, ...args) {
...
let unicode_hack_impl = /*@__PURE__*/ (function () {
...
export function trim(target, ...rest) {
    if (rest.length == 0)
        return target.trim();
    var parts = stringSplit(target, rest, 0 /* None */);
    return parts.join("");
}
export function trimStart(target, ...rest) {
    if (target.length === 0) {
        return target;
    }
    var args;
...
export function trimEnd(target, ...rest) {
    if (target.length === 0) {
        return target;
    }
    var args;
